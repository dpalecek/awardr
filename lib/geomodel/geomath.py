...
from lib.geomodel import geotypes
...
def distance(p1, p2):
  ...
  return RADIUS * math.acos(max(min(math.sin(p1lat) * math.sin(p2lat) + math.cos(p1lat) * math.cos(p2lat) * math.cos(p2lon - p1lon),1),-1))
  '''
  Below is broken.  See: http://code.google.com/p/geomodel/issues/detail?id=13
