...
class UserSetCodeRateLookup(db.Model):
    ...
    @classmethod
    def calc_key_name(cls, user, hotel_id, check_in, check_out):
        return "%s_%d-%s-%s" % (cls.kind(), hotel_id, helper.date_to_str(check_in), helper.date_to_str(check_out))
...
class StarwoodRateLookup(db.Model):
    ...
    def create(hotel, ratecode_entity, date):
        ...
        starwood_ratecode_lookup = StarwoodRateLookup(key_name=key_name, hotel=hotel, ratecode=ratecode_entity, date=date)
